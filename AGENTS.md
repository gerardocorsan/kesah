...
- `src/model/` never imports from `solid-js` or touches the DOM. Measurements are passed in as numbers.
...
- Mock only real external dependencies (browser dialogs, object URLs, timers). jsdom stand-ins live in
  `src/test/setup.ts`. Never mock the unit under test.
- Every unit has an entry in `scripts/verify-tests.mjs`; a mutation that survives means the test is
...
- `./start` runs the dev server on port 5173; do not leave your own dev servers running on it.
- Temporary files go under `tmp/` (ignored), never in `src/`.
