...
const MUTATIONS = [
  ...
  { unit: 'graph.setEdgeKind', file: 'src/model/graph.ts', what: 'keeps the condition on message flows', find: "edge.condition = 'none';\n      delete edge.expression;", replace: 'delete edge.expression;', tests: ['src/model/graph.test.ts'] },
  ...
  { unit: 'graph.setNodeScript', file: 'src/model/graph.ts', what: 'blank text kept', find: "const next = typeof value === 'string' && value.trim() !== '' ? value : undefined;", replace: 'const next = value;', tests: ['src/model/graph.test.ts'] },
  { unit: 'graph.setNodeDelay', file: 'src/model/graph.ts', what: 'negative delays kept', find: 'const next = isFiniteNumber(delay) && delay >= 0 ? delay : undefined;', replace: 'const next = isFiniteNumber(delay) ? delay : undefined;', tests: ['src/model/graph.test.ts'] },
  { unit: 'graph.setEdgeKind (expression)', file: 'src/model/graph.ts', what: 'expression kept on message flows', find: "edge.condition = 'none';\n      delete edge.expression;", replace: "edge.condition = 'none';", tests: ['src/model/graph.test.ts'] },
  { unit: 'graph.parse (delay)', file: 'src/model/graph.ts', what: 'negative delay imported', find: 'if (isFiniteNumber(raw.delay) && raw.delay >= 0) node.delay = raw.delay;', replace: 'if (isFiniteNumber(raw.delay)) node.delay = raw.delay;', tests: ['src/model/graph.test.ts'] },
  { unit: 'graph.parse (expression)', file: 'src/model/graph.ts', what: 'expression imported on any kind', find: "if (kind === 'sequence' && typeof raw.expression === 'string' && raw.expression.trim() !== '') edge.expression = raw.expression;", replace: "if (typeof raw.expression === 'string' && raw.expression.trim() !== '') edge.expression = raw.expression;", tests: ['src/model/graph.test.ts'] },
  ...
  { unit: 'InspectorPanel (script field)', file: 'src/components/organisms/InspectorPanel.tsx', what: 'script shown for gateways', find: "return n !== undefined && (n.type === 'task' || n.type === 'subprocess');", replace: 'return n !== undefined;', tests: ['src/components/organisms/InspectorPanel.test.tsx'] },
  { unit: 'InspectorPanel (delay field)', file: 'src/components/organisms/InspectorPanel.tsx', what: 'delay shown for message events', find: "return n !== undefined && isEvent(n.type) && n.variant === 'timer';", replace: 'return n !== undefined && isEvent(n.type);', tests: ['src/components/organisms/InspectorPanel.test.tsx'] },
  { unit: 'InspectorPanel (delay value)', file: 'src/components/organisms/InspectorPanel.tsx', what: 'delay never stored', find: "app.graph.setNodeDelay(n.id, trimmed === '' ? undefined : Number(trimmed));", replace: ';', tests: ['src/components/organisms/InspectorPanel.test.tsx'] },
  { unit: 'InspectorPanel (message value)', file: 'src/components/organisms/InspectorPanel.tsx', what: 'message never stored', find: 'if (n) app.graph.setNodeMessage(n.id, value);', replace: ';', tests: ['src/components/organisms/InspectorPanel.test.tsx'] },
  { unit: 'InspectorPanel (expression field)', file: 'src/components/organisms/InspectorPanel.tsx', what: 'expression shown for associations', find: "const hasExpression = () => edge()?.kind === 'sequence';", replace: 'const hasExpression = () => edge() !== undefined;', tests: ['src/components/organisms/InspectorPanel.test.tsx'] },
  { unit: 'InspectorPanel (execution undo step)', file: 'src/components/organisms/InspectorPanel.tsx', what: 'typing into execution fields not grouped', find: 'const beginStep = () => app.history.begin();', replace: 'const beginStep = () => undefined;', tests: ['src/components/organisms/InspectorPanel.test.tsx'] },
  { unit: 'InspectorPanel (scroll into view)', file: 'src/components/organisms/InspectorPanel.tsx', what: 'selection does not reveal the inspector', find: "if (selection) section.scrollIntoView({ block: 'nearest' });", replace: ';', tests: ['src/components/organisms/InspectorPanel.test.tsx'] },
  ...
  { unit: 'main (executable sample)', file: 'src/main.tsx', what: 'sample gateway has no expression', find: "if (yes) graph.setEdgeExpression(yes.id, 'vars.stock');", replace: ';', tests: ['src/main.test.tsx'] },
